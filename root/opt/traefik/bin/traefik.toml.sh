...
TRAEFIK_KV_PREFIX=${TRAEFIK_KV_PREFIX:-"/traefik"}
...
TRAEFIK_KV_CFG=""
if [ "${TRAEFIK_KV_ENABLE}" == "true" ]; then
    if [ "${TRAEFIK_KV_TYPE}" == "etcd" ]; then
        TRAEFIK_KV_PORT=${TRAEFIK_KV_PORT:-"2379"}
        TRAEFIK_KV_CFG="
[etcd]
  endpoint = \"${TRAEFIK_KV_ENDPOINT}:${TRAEFIK_KV_PORT}\"
  watch = true
  prefix = \"${TRAEFIK_KV_PREFIX}\"
  useAPIV3 = true
"
    fi

    if [ "${TRAEFIK_KV_TYPE}" == "consul" ]; then
        TRAEFIK_KV_PORT=${TRAEFIK_KV_PORT:-"8500"}
        TRAEFIK_KV_CFG="
[consul]
  endpoint = \"${TRAEFIK_KV_ENDPOINT}\"
  watch = true
  prefix = \"${TRAEFIK_KV_PREFIX}\"
"
    fi
